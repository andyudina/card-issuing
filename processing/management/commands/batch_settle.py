...
from processing.apis import SchemaAPI, \
                            TelegramAPI, \
                            SmsAPI, \
                            SendgridAPI
...
class Command(BaseCommand):
    ...
    def _transfer_settlements_to_schema(self):
        ...
        '''
        inner_settlement_account = UserAccountsUnion.objects.get_inner_settlement_account()
        external_settlement_account = UserAccountsUnion.objects.get_external_settlement_account()
        try:
    ...
    def _log_settlement_transaction(self, **kwargs):
        ...
        '''
        # so error-prone logic. should be rewrighten after requirements are specified
    ...
    def _rollback_outdated_transactions(self):
        ...
        '''
        transactions_date_treshold = self._get_transactions_date_treshold()
        outdated_transaction_codes = Transaction.objects.get_non_presented_transactions_before(
    ...
    def _get_transactions_date_treshold(self):
        ...
        ''' 
        return to_start_day(datetime.datetime.now() - \
