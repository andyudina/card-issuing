...
from processing.models.transactions import IssuerTransactionError, \
                                    TRANSACTION_PRESETMENT_STATUS, TRANSACTION_PRESENTMANT_IS_TOO_LATE_STATUS
...
class TransactionTransferManagement(CreateAccountMixin, CreateTransactionMixin, 
    ...
    def test__add_transfer(self):
        self.transaction.add_transafer(self.user_account.base_account, 
...
class AuthorisationTransaction(CreateAccountMixin, CreateTransactionMixin, 
    ...
    def setUpTestData(cls):
        cls.user_account = cls.create_account_with_amount()
        cls.transfer_sum = 0.5 * cls.user_account.base_account.amount 
    ...
    def test__already_created(self):
        ...
        err = err_cm.exception
        self.assertEqual(err.args[0], 'already_created')        


class PresentmentTransaction(TestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        cls.user_account_1 = cls.create_account_with_amount()
        cls.user_account_2 = cls.create_account()
        cls.revenue_account = cls.create_account()
        cls.base_sum = cls.user_account_1.base_account.amount
        cls.transfer_sum = 0.5 * cls.user_account_1.base_account.amount
        cls.transaction = cls.create_transaction(
            from_account=cls.user_account_1.base_account,
            to_account=cls.user_account_1.reserved_account,
    ...
    def test__transaction_is_ok__without_extra_transfers(self):
        transaction = Transaction.objects.present_transaction(
            cls.transaction.code, self.transfer_sum, self.transfer_sum,
        ...
        self.check_successfull_money_transfering(account_id=self.user_account_1.base_account.id,
            expected_sum=-self.base_sum - self.transfer_sum, transaction_id=self.transaction.id, 
            transfer_sum=self.transfer_sum)
    ...
    def test__transaction_is_ok__with_extra_transfers(self):
        settlement_coeff = 0.7
        transaction = Transaction.objects.present_transaction(
            cls.transaction.code, self.transfer_sum, settlement_coeff * self.transfer_sum,
            from_account=self.user_account_1.base_account,
            to_account=self.user_account_2.base_account,
            extra_account=self.revenue_account
        )
        # check user_1 base amount was deducted
        self.check_successfull_money_transfering(account_id=self.user_account_1.base_account.id,
            expected_sum=self.base_sum - self.transfer_sum, transaction_id=self.transaction.id, 
        ...
        # check user_2 base amount was added
        self.check_successfull_money_transfering(account_id=self.user_account_2.base_account.id,
            expected_sum=settlement_coeff * self.transfer_sum, transaction_id=self.transaction.id, 
        ...
        self.check_successfull_money_transfering(account_id=self.revenue_account.base_account.id,
            expected_sum=(1 - settlement_coeff) * self.transfer_sum, transaction_id=self.transaction.id, 
            transfer_sum=(1 - settlement_coeff) * self.transfer_sum)

    def test__transaction_is_already_done(self):
        self.create_transaction(
            code=self.transaction.code, status=TRANSACTION_PRESETMENT_STATUS,
            from_account=self.user_account_1.base_account,
            to_account=self.user_account_2.base_account,
        )
        ...
        with self.assertRaises(IssuerTransactionError) as err_cm:
            Transaction.objects.present_transaction(
                self.transfer_sum, self.transfer_sum,
                from_account=self.user_account_1.base_account,
                to_account=self.user_account_2.base_account,
            )
        err = err_cm.exception
...
class RollbackNonPresentmentTransaction(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.transfer_sum = 0.2 * cls.base_sum
        ...
        cls.create_transaction(
            ...
            amount=cls.transfer_sum)
        cls.transaction_ids = {transaction.id: 1 for transaction in cls.transactions[1:]}


    def test__get_transactions_for_rollback(self):
        tomorrow = datetime.datetime.now() + datetime.timedelta(days=1)
        transactions = Transaction.objects.get_non_presented_transactions_before(tomorrow)
        transaction_ids = {transaction.id: 1 for transaction in transactions}
        self.assertDictEqual(transaction_ids, cls.transaction_ids)

    def test__transaction_are_rollbacked_already(self):
        cls.create_transaction(
            code=self.transaction[1].code,
            status=TRANSACTION_PRESENTMANT_IS_TOO_LATE_STATUS
        )
        Transaction.objects.rollback_late_presentment(self.transaction[1].code)
        self.assertEqual(self.user_account.base_account.amount, self.base_sum)


    def test__transaction_is_rollbacked_successfully(self):
        transaction = Transaction.objects.rollback_late_presentment(self.transaction[2].code)
        ...
        self.check_successfull_money_transfering(account_id=self.user_account.reserved_account.id,
            expected_sum=0, transaction_id=transaction.id, 
            transfer_sum=-self.transfer_sum)