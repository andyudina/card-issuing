...
from apps.users.serializers import BalanceRequestSerializer, \
...
class TransactionsView(GetUserAccountMixin, 
    ...
    def filter_queryset(self, *args, **kwargs):
        ...
        request_serializer = TransactionRequestSerializer(data=self.request.query_params)
...
class BalanceView(GetUserAccountMixin, 
    ...
    def get(self, request, *args, **kwargs):
        user_account = self.get_object()
        request_serializer = BalanceRequestSerializer(data=request.query_params)
        if request_serializer.is_valid():
